...
func (t *typeValidator) Validate(data interface{}) *Result {
	...
	isIntOrString := schType == "integer" && t.Format == "int-or-string"

	if kind != reflect.String && kind != reflect.Slice && t.Format != "" && !(t.Type.Contains(schType) || format == t.Format || isFloatInt || isIntFloat || isLowerInt || isLowerFloat || isIntOrString) {
		// TODO: test case
		return errorHelp.sErr(errors.InvalidType(t.Path, t.In, t.Format, format))
	}

	if !(t.Type.Contains("number") || t.Type.Contains("integer")) && t.Format != "" && (kind == reflect.String || kind == reflect.Slice || kind == reflect.Int64) {
		return result
	}
